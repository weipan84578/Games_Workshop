  ...
  for (let step = 0; step < bars * 8; step += 1) {
    ...
    chord.forEach((midi, voice) => {
      ...
      for (let i = 0; i < count && start + i < samples.length; i += 1) {
        ...
        const tone = Math.sin(2 * Math.PI * frequency * time) + 0.31 * Math.sin(4 * Math.PI * frequency * time) + 0.12 * Math.sin(6 * Math.PI * frequency * time);
...
function sfx(type) {
  ...
  const tones = { click: 880, confirm: 1047, cancel: 440, error: 196, coin: 1319, equip: 740, countdown: 660, hit: 180, miss: 520, gold: 1568, bond: 988, attack: 240, special: 360, dodge: 720, critical: 1320, shield: 560, victory: 784, defeat: 220, xp: 880, level: 1047, rank: 1175, champion: 1568 };
  ...
  for (let i = 0; i < length; i += 1) {
    ...
    if (type === 'victory' || type === 'level' || type === 'champion') f *= Math.pow(2, Math.floor(t / 0.16) / 12 * 4);
...
const tracks = {
  bgm_menu: { tempo: 92, notes: [[60,64,67],[64,67,72],[67,71,74],[65,69,72]], bars: 4 },
  bgm_home: { tempo: 84, notes: [[60,64,67],[62,65,69],[59,62,67],[60,65,69]], bars: 4 },
  bgm_training: { tempo: 116, notes: [[60,64,67],[62,66,69],[64,67,71],[67,71,74]], bars: 5 },
  bgm_outing: { tempo: 104, notes: [[65,69,72],[67,71,74],[64,69,72],[62,67,71]], bars: 5 },
  bgm_battle: { tempo: 142, notes: [[57,60,64],[59,62,65],[60,64,67],[55,59,62]], bars: 6 },
  bgm_champion: { tempo: 96, notes: [[60,64,67,72],[65,69,72,77],[67,71,74,79],[60,67,72,76]], bars: 6 }
};
const sfxNames = ['click','confirm','cancel','error','coin','equip','countdown','hit','miss','gold','bond','attack','special','dodge','critical','shield','victory','defeat','xp','level','rank','champion'];
fs.mkdirSync(path.join(ROOT, 'assets/audio/bgm'), { recursive: true });
fs.mkdirSync(path.join(ROOT, 'assets/audio/sfx'), { recursive: true });
Object.entries(tracks).forEach(([name, data]) => fs.writeFileSync(path.join(ROOT, 'assets/audio/bgm', name + '.wav'), wav(pianoTrack(data.notes, data.tempo, data.bars))));

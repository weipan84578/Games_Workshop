...
const gear = {
  ...
  guard:
    '<path d="M32 10 51 17v14c0 12-8 20-19 25C21 51 13 43 13 31V17l19-7Zm0 9-10 4v8c0 7 4 12 10 16 6-4 10-9 10-16v-8l-10-4Z" fill-rule="evenodd"/>',
  ...
  fortune:
    '<path d="M32 8 43 21 32 32 21 21 32 8Zm0 24 11 11-11 13-11-13 11-11ZM8 32l13-11 11 11-11 11L8 32Zm24 0 11-11 13 11-13 11-11-11Z"/> '
};
const consumables = {
  energy: '<path d="M24 9h16v8l6 7v30H18V24l6-7V9Zm5 18-5 13h8l-2 10 10-16h-8l4-7h-7Z" fill-rule="evenodd"/>',
  shield:
    '<path d="M13 25c0-8 8-13 19-13s19 5 19 13v17c0 8-8 12-19 12s-19-4-19-12V25Zm9 3h20v7H22v-7Z" fill-rule="evenodd"/>',
  focus:
    '<path d="m8 24 10-9 7 5h14l7-5 10 9-9 9 9 9-10 9-7-5H25l-7 5-10-9 9-9-9-9Zm24 2a7 7 0 1 0 0 14 7 7 0 0 0 0-14Z" fill-rule="evenodd"/>'
};
...
for (let stage = 1; stage <= 6; stage += 1) {
  Object.entries(gear).forEach(([key, shape]) =>
    fs.writeFileSync(path.join(out, `eq_${stage}_${key}.svg`), svg(stages[stage - 1], shape, '#30243a'))
  );
  Object.entries(consumables).forEach(([key, shape]) =>
    fs.writeFileSync(path.join(out, `con_${stage}_${key}.svg`), svg(stages[stage - 1], shape, '#fff0b8'))
  );
}

...
class FakeAudioContext {
  ...
  createDynamicsCompressor() { return { threshold: new FakeParam(), knee: new FakeParam(), ratio: new FakeParam(), attack: new FakeParam(), release: new FakeParam(), connect(node) { return node; } }; }
...
function loadManager(protocol) {
  ...
  const window = {
    ...
    PSG: {
      ...
      storage: { save: { settingsDefaults: () => ({ masterVolume: 0.5, bgmVolume: 0.3, sfxVolume: 0.65, muted: false }) } }
    }
  };
  window.window = window;
  vm.runInNewContext(fs.readFileSync(path.join(__dirname, '..', 'js/audio/audioManager.js'), 'utf8'), window, { filename: 'js/audio/audioManager.js' });

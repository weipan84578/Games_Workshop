...
check('rank-one Boss gate loads safely and uses the endless battle rules', () => {
  ...
  const locales = fs.readFileSync(path.join(root, 'js/i18n/bossLocales.js'), 'utf8');
  ...
  assert.match(boss, /MIRROR_MIN_VARIANCE\s*=\s*0\.01/);
  assert.match(boss, /MIRROR_MAX_VARIANCE\s*=\s*0\.05/);
  assert.match(boss, /function createMirror\(save\)/);
  assert.match(boss, /function settleMirror\(save, challenge, won\)/);
  ...
  assert.match(bossUi, /data-mirror-boss/);
  assert.match(bossUi, /createMirror\(save\)/);
  assert.match(bossUi, /boss-card__mirror-icon/);
  assert.match(bossUi, /boss\.mirror\.hiddenOpponent/);
  assert.doesNotMatch(bossUi, /mirrorOpponent|mirrorSpecies|boss\.mirror\.randomOpponent/);
  assert.match(locales, /boss\.mirror\.name/);
  ...
  assert.match(battle, /function isMirrorBattle\(state\)/);
  assert.match(battle, /settleMirror\(save, state\.bossChallenge, won\)/);
  ...
  assert.match(battleUi, /result\.mirror/);

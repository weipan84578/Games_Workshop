(function (root) {
  ...
  Bgm.start = function (name) {
    ...
    function schedule() {
      ...
      var octave = index % 4 === 3 ? 0.5 : 1;
      audio.playBgmNote(
        note * octave,
        selected === "victory" ? 0.3 : 0.34,
        selected === "victory" ? 0.055 : 0.038,
        "triangle",
      );
      if (index % 4 === 0) audio.playBgmNote(note / 2, 0.62, 0.018, "sine");
      index += 1;
  ...
  Bgm.pickBattle = function () {
    ...
    if (selected === previousBattle)
      selected = choices[(choices.indexOf(selected) + 1) % choices.length];

(function (root) {
  ...
  Audio.init = function () {
    ...
    try {
      ...
      state.bgmCompressor.attack.value = 0.004;
      state.bgmCompressor.release.value = 0.22;
  ...
  Audio.applySettings = function (settings) {
    ...
    state.master.gain.setTargetAtTime(cg.Utils.clamp(master, 0, 1), now, 0.025);
    state.bgmInput.gain.setTargetAtTime(cg.Utils.clamp(bgm, 0, 10), now, 0.06);
    state.sfx.gain.setTargetAtTime(cg.Utils.clamp(sfx, 0, 1), now, 0.025);
  };
  Audio.effectiveBgmGain = function () {
    return (root.GameState.settings.bgmVolume || 0) * 10;
  };
  Audio.setPaused = function (paused) {
    state.bgmPaused = paused;
    if (!state.ready) return;
    state.bgmInput.gain.setTargetAtTime(
      paused
        ? 0.06
        : cg.Utils.clamp(root.GameState.settings.bgmVolume * 10, 0, 10),
  ...
  Audio.playBgmNote = function (frequency, duration, velocity, wave) {
    if (
      !state.ready ||
      state.bgmPaused ||
      !root.GameState.settings.bgmEnabled ||
  ...
  Audio.playTone = function (
    ...
    if (
      !state.ready ||
      (!isBgm && state.activeSfx >= cg.Constants.MAX_SIMULTANEOUS_SFX)
    )
      return;
    ...
    osc.addEventListener("ended", function () {
      if (!isBgm) state.activeSfx = Math.max(0, state.activeSfx - 1);
      osc.disconnect();
      gain.disconnect();
    });
  };
  Audio.playSfx = function (name, intensity) {
    if (cg.Sfx && cg.Sfx.play) cg.Sfx.play(name, intensity);
  };

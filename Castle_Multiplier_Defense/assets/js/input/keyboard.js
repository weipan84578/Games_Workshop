(function (root) {
  ...
  Input.on = function (action, callback) {
    (Input.listeners[action] = Input.listeners[action] || []).push(callback);
  };
  Input.emit = function (action, payload) {
    (Input.listeners[action] || []).slice().forEach(function (callback) {
      callback(payload);
    });
  };
  document.addEventListener("keydown", function (event) {
    ...
    if (
      ...
      (root.GameState.battle && root.GameState.battle.paused)
    )
      return;
    if (code === "Space") {
      event.preventDefault();

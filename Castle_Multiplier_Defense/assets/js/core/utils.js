(function (root) {
  ...
  U.inverseLerp = function (a, b, value) {
  ...
  U.rectContains = function (rect, x, y) {
    return (
  ...
  U.formatNumber = function (value) {
    try {
      return Math.round(value).toLocaleString(
        (root.GameState && root.GameState.locale) || "zh-TW",
      );
    } catch (error) {
      return String(Math.round(value));
    }
  };
  U.formatPercent = function (value) {
    return Math.round(U.clamp(value, 0, 1) * 100) + "%";
  };
  ...
  U.getDpr = function () {
    return Math.min(window.devicePixelRatio || 1, cg.Constants.MAX_DPR);
  };
  U.getQuality = function () {
    var setting =
      root.GameState &&
      root.GameState.settings &&
      root.GameState.settings.graphicsQuality;
    if (setting && setting !== "auto") return setting;
    if (U.isTouchDevice()) return "medium";
    return "high";
  };
  U.safeJsonParse = function (value) {
    try {
  ...
  U.segmentLength = function (x1, y1, x2, y2) {
    return Math.hypot(x2 - x1, y2 - y1);
  };
  U.round = function (value, digits) {

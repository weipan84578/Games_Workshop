(function (root) {
  ...
  Loop.frame = function (time) {
    if (!Loop.running) return;
    var delta = Math.min(
      cg.Constants.MAX_DELTA,
      Math.max(0, (time - Loop.last) / 1000),
    );
    Loop.last = time;
    Loop.frameTime = delta;
    Loop.fps = delta > 0 ? cg.Utils.lerp(Loop.fps, 1 / delta, 0.08) : Loop.fps;
    if (
      root.GameState.screen === cg.Constants.SCREENS.GAME &&
      cg.Battle &&
      cg.Battle.active
    )
      cg.Battle.update(delta);
    if (
      cg.Battle &&
      (root.GameState.screen === cg.Constants.SCREENS.GAME ||
    ...
    if (
      ...
      cg.App.updateDebug(Loop.fps, delta);
    requestAnimationFrame(Loop.frame);
  };
})(window);
